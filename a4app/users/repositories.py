...
class AuthRepository(AuthRepositoryInterface):
    ...
    async def save_user(self, email: str, username: str, password: str, is_admin: bool = False):
        result = await self._session.execute(insert(User).values(email=email, password=password, username=username, is_admin=is_admin).returning(User))
